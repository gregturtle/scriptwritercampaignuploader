...
class AIScriptService {
  ...
  async generateScriptSuggestions(
    ...
    try {
      ...
      // Force 38-44 word scripts for maximum 13-15 seconds
      const targetWordCount = "38-44";
      console.log(`Targeting ${targetWordCount} words for maximum 13-15 second scripts`);
...
- MANDATORY: Each script must be EXACTLY 38-44 words (maximum 13-15 seconds when spoken)
...
- Must be EXACTLY 38-44 words (NEVER exceed 13-15 seconds when spoken naturally)
...
- CRITICAL: Count every word carefully - scripts over 44 words will be rejected
...
✓ Total script is no more than 44 words
✓ The script would take no more than 13-15 seconds to say out loud at normal advertising pace
✓ what three words does not give directions, only provides precise locations that people navigate to
